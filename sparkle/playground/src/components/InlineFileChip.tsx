...
>(({ label, fileType, onClick, className }, ref) => {
  ...
  return (
    <button
      ...
      title={label}
      className={cn(
        "s-inline s-cursor-pointer s-border-0 s-bg-transparent s-p-0",
        "s-font-medium s-text-highlight-500 dark:s-text-highlight-500-night",
        "hover:s-underline",
        className
      )}
    >
      <Icon
        ...
        size="xs"
        className="s-mr-0.5 s-inline-block s-align-[-0.15em]"
      />
      {label}
    </button>
  );

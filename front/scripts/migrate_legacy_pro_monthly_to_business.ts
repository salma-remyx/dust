 ...
 *      Stripe subscription to cancel at the same moment (no double-billing).
 *   4. Sets the legacy subscription's `endDate` (and `requestCancelAt`) to the
 *      migration date so the upcoming end is reflected in the DB / UI.
 *
 ...
 *     [--windowStart 2026-07-23] [--windowEnd 2026-08-23] [--now] \
 *     [--workspaceId <sId>] [--concurrency 4] [--execute]
 *
 * `--now` migrates immediately (current hour) instead of the workspace's
 * billing-date window boundary (useful for testing a single --workspaceId).
 */
...
async function migrateWorkspace(
  ...
  // `--now` migrates immediately, at the current hour boundary. The contract is
  // backdated so Metronome fires `contract.start` right away; with no pending
  // row staged the handler swaps the active subscription in place. Otherwise use
  // the workspace's own renewal boundary within the rollout window.
  ...
  // Reflect the scheduled end on the legacy subscription. switchContract has
  // already scheduled the Stripe cancellation and created the pending Metronome
  // subscription that will take over at the same moment. The credit conversion
  // and free bonus are applied by the contract.start webhook at activation.
  await subscription.markAsCanceled({ endDate: migrationDate });
  ...
  logger.info(
    ...
    "[migrate-business] Migrated: pending Business contract scheduled + legacy subscription end date set"
  );
}
...
makeScript(
  {
    ...
    now: {
      ...
      description:
        "Migrate immediately (current hour) instead of the workspace's billing-date window boundary",
      default: false,
    },
    workspaceId: {
      type: "string" as const,
  ...
  async (
    { windowStart, windowEnd, now, workspaceId, concurrency, execute },
    logger
  ) => {
